...
export default function VocabForm({ initialData, onSubmit, submitLabel }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">日文（漢字）*</label>
        <input
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">假名（讀音）*</label>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl border border-gray-200 hover:bg-gray-50 text-lg"
            title="朗讀"
          >
            🔊
          </button>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">例句（日文）</label>
        <input
          className={inputClass}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">例句（中文）</label>
        <input
          className={inputClass}
          ...
          <div className="flex gap-1">
            {PRESET_COLORS.map((color) => (
              <button
                ...
                className={`w-6 h-6 rounded-full transition-transform ${newCatColor === color ? 'scale-125 ring-2 ring-offset-1 ring-gray-400' : ''}`}
